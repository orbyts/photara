...
struct PhotaraGraphCanvas<NodeContent: View, ToolRail: View>: View {
    @Environment(\.accessibilityReduceTransparency) private var reduceTransparency
    let toolRailPlacement: PhotaraGraphToolRailPlacement
    ...
    var body: some View {
        let railPlacement = toolRailPlacement
        ...
        ZStack {
            PhotaraGraphCanvasBackground(controller: controller, style: backgroundStyle, backgroundColor: backgroundColor, minorColor: minorColor, majorColor: majorColor)
            PhotaraGraphCanvasNoodles(controller: controller, color: noodleColor)
            ForEach(controller.document.nodes) { node in
                PhotaraGraphPlacedNode(controller: controller, node: node, content: nodeContent)
            }
        ...
        .overlay(alignment: overviewPosition.alignment) {
            PhotaraGraphOverviewOverlay(controller: controller, position: overviewPosition)

        }
        .overlay(alignment: .bottom) {
            PhotaraGraphZoomControl(controller: controller, reduceTransparency: reduceTransparency).padding(14)
        }
...
private struct PhotaraGraphCanvasBackground: View {
    let controller: PhotaraGraphInteractionController
...
private struct PhotaraGraphPlacedNode<Content: View>: View {
    let controller: PhotaraGraphInteractionController
...
private struct PhotaraGraphCanvasNoodles: View {
    let controller: PhotaraGraphInteractionController
    let color: Color
    var body: some View {
...
private struct PhotaraGraphZoomControl: View {
    let controller: PhotaraGraphInteractionController
...
private struct PhotaraGraphOverviewOverlay: View {
    let controller: PhotaraGraphInteractionController
