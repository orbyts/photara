#!/bin/zsh
set -euo pipefail

SCRIPT_ROOT="${0:A:h}"
REPOSITORY_ROOT="${SCRIPT_ROOT:h:h:h}"
BUILD_ROOT="$SCRIPT_ROOT/.build/bridge-verification"
RUST_TARGET="${PHOTARA_BRIDGE_RUST_TARGET:-$BUILD_ROOT/rust-target}"
GENERATED_ROOT="$BUILD_ROOT/generated"
MODULE_CACHE="$BUILD_ROOT/module-cache"
EXECUTABLE="$BUILD_ROOT/photara-bridge-verification"
PROXY_HELPER_BUILD="$BUILD_ROOT/proxy-helper-build"
PROXY_HELPER="$PROXY_HELPER_BUILD/debug/photara-proxy-imageio"

mkdir -p "$GENERATED_ROOT" "$MODULE_CACHE"

swift build \
  --package-path "$REPOSITORY_ROOT/platform/macos/photara-proxy-imageio" \
  --scratch-path "$PROXY_HELPER_BUILD"

CARGO_TARGET_DIR="$RUST_TARGET" cargo build \
  --manifest-path "$REPOSITORY_ROOT/Cargo.toml" \
  -p photara-bridge

CARGO_TARGET_DIR="$RUST_TARGET" cargo run \
  --manifest-path "$REPOSITORY_ROOT/Cargo.toml" \
  -p photara-bridge \
  --features bindgen \
  --bin photara-uniffi-bindgen \
  -- generate \
  --library "$RUST_TARGET/debug/libphotara_bridge.dylib" \
  --language swift \
  --out-dir "$GENERATED_ROOT"

xcrun swiftc \
  -swift-version 6 \
  -parse-as-library \
  -module-cache-path "$MODULE_CACHE" \
  "$GENERATED_ROOT/PhotaraBridge.swift" \
  "$REPOSITORY_ROOT/platform/macos/photara-graph/Sources/GraphDocument.swift" \
  "$SCRIPT_ROOT/Sources/GraphAdapter.swift" \
  "$SCRIPT_ROOT/Sources/EditorSessionModel.swift" \
  "$SCRIPT_ROOT/BridgeVerification/main.swift" \
  -Xcc "-fmodule-map-file=$GENERATED_ROOT/PhotaraBridgeFFI.modulemap" \
  -L "$RUST_TARGET/debug" \
  -lphotara_bridge \
  -framework SwiftUI \
  -framework AppKit \
  -o "$EXECUTABLE"

DYLD_LIBRARY_PATH="$RUST_TARGET/debug" "$EXECUTABLE" "$PROXY_HELPER"
