...
impl LocalLibraryStore {
    /// Creates or CAS-updates a local Workspace. Retirement never cascades.
    /// # Errors
    ...
    pub async fn put_workspace(
        ...
        let m = &value.meta;
        if m.id != m.workspace_id {
            return Err(Error::Invalid);
        }
        let mut tx = self.write().await?;
        check_meta(&mut tx, Table::Workspace, m, expected).await?;
        if m.state == Lifecycle::Tombstoned {
            for table in [
                ...
                let sql = sqlx::AssertSqlSafe(format!(
                    "SELECT EXISTS(SELECT 1 FROM {} WHERE workspace_id=? AND state='active')",
                    table.names().0
                ));
            ...
            if sqlx::query_scalar::<_,bool>("SELECT EXISTS(SELECT 1 FROM project_catalog WHERE workspace_id=? AND visibility='visible')").bind(m.id.bytes()).fetch_one(&mut *tx).await? {return Err(Error::Constraint);}
        }
        sqlx::query(if m.revision==Revision::INITIAL {"INSERT INTO workspaces(workspace_id,display_name,state,local_revision,created_at_ms,updated_at_ms,retired_at_ms,term_policy_version,extensions_json) VALUES(?,?,?,?,?,?,?,1,?)"} else {"UPDATE workspaces SET workspace_id=?1,display_name=?2,state=?3,local_revision=?4,created_at_ms=?5,updated_at_ms=?6,retired_at_ms=?7,term_policy_version=1,extensions_json=?8 WHERE workspace_id=?1 AND workspace_id=?1"})
    ...
    pub async fn workspace(&self, id: WorkspaceId) -> Result<Option<Workspace>> {
        ...
        .await?;
        rows.first().map(workspace).transpose()
    }
    /// Lists local Workspaces by stable UUID, including tombstones only on request.
    /// # Errors
    ...
    pub async fn workspaces(
        ...
        page(0, limit)?;
        let rows=sqlx::query("SELECT * FROM workspaces WHERE workspace_id>? AND (? OR state='active') ORDER BY workspace_id LIMIT ?")
            .bind(after.map_or_else(Vec::new,WorkspaceId::bytes)).bind(retired).bind(limit).fetch_all(self.db.pool()).await?;
    ...
    pub async fn put_person(
        ...
        sqlx::query("DELETE FROM person_capabilities WHERE workspace_id=? AND person_id=?")
            .bind(value.meta.workspace_id.bytes())
    ...
    /// # Errors
    /// Returns validation, CAS or same-Workspace/live-reference/interval errors.
    pub async fn put_relationship(
        ...
        sqlx::query(if m.revision==Revision::INITIAL {"INSERT INTO person_organization_relationships(relationship_id,workspace_id,person_id,organization_id,relationship_type,valid_from_ms,valid_until_ms,notes,labels_json,record_schema,local_revision,created_at_ms,updated_at_ms,state,retired_at_ms,extensions_json) VALUES(?,?,?,?,?,?,?,?,?,1,?,?,?,?,?,?)"} else {"UPDATE person_organization_relationships SET relationship_id=?1,workspace_id=?2,person_id=?3,organization_id=?4,relationship_type=?5,valid_from_ms=?6,valid_until_ms=?7,notes=?8,labels_json=?9,record_schema=1,local_revision=?10,created_at_ms=?11,updated_at_ms=?12,state=?13,retired_at_ms=?14,extensions_json=?15 WHERE relationship_id=?1 AND workspace_id=?2"})
            .bind(m.id.bytes()).bind(m.workspace_id.bytes()).bind(value.person_id.bytes()).bind(value.organization_id.bytes()).bind(&value.relationship_type).bind(value.valid_from.map(Timestamp::get)).bind(value.valid_until.map(Timestamp::get)).bind(&value.notes).bind(canonical(&value.labels)?).bind(m.revision.get()).bind(m.created_at.get()).bind(m.updated_at.get()).bind(m.state.sql()).bind(m.retired_at()).bind(canonical(&value.extensions)?).execute(&mut *tx).await?;
        let result = finish(
            &mut tx,
            self.info.device_id,
            Table::Relationship,
    ...
    pub async fn put_location_kind(
        ...
        for (old_key, spellings) in terms(&mut tx, m.workspace_id, m.id).await? {
        ...
        sqlx::query(if m.revision==Revision::INITIAL {"INSERT INTO location_kinds(location_kind_id,workspace_id,canonical_key,canonical_display,description,record_schema,local_revision,created_at_ms,updated_at_ms,state,retired_at_ms,extensions_json,retirement_terms_json) VALUES(?,?,?,?,?,1,?,?,?,?,?,?,?)"} else {"UPDATE location_kinds SET location_kind_id=?1,workspace_id=?2,canonical_key=?3,canonical_display=?4,description=?5,record_schema=1,local_revision=?6,created_at_ms=?7,updated_at_ms=?8,state=?9,retired_at_ms=?10,extensions_json=?11,retirement_terms_json=?12 WHERE location_kind_id=?1 AND workspace_id=?2"})
            .bind(m.id.bytes()).bind(m.workspace_id.bytes()).bind(&key).bind(&value.canonical_display).bind(&value.description).bind(m.revision.get()).bind(m.created_at.get()).bind(m.updated_at.get()).bind(m.state.sql()).bind(m.retired_at()).bind(canonical(&value.extensions)?).bind(retirement).execute(&mut *tx).await?;
        for (term, spellings) in &claims {
            let count=sqlx::query("INSERT INTO location_kind_terms(workspace_id,term_key,location_kind_id,policy_version,spellings_json) VALUES(?,?,?,1,?) ON CONFLICT(workspace_id,term_key) DO UPDATE SET spellings_json=excluded.spellings_json WHERE location_kind_terms.location_kind_id=excluded.location_kind_id")
                .bind(m.workspace_id.bytes()).bind(term).bind(m.id.bytes()).bind(canonical(spellings)?).execute(&mut *tx).await?.rows_affected();
            if count != 1 {
    ...
    pub async fn resolve_location_kind(
        ...
        let mut tx = self.read().await?;
        let row=sqlx::query("SELECT k.* FROM location_kind_terms t JOIN location_kinds k ON k.workspace_id=t.workspace_id AND k.location_kind_id=t.location_kind_id WHERE t.workspace_id=? AND t.term_key=?")
            .bind(workspace.bytes()).bind(key).fetch_optional(&mut *tx).await?;
        match row {
            Some(row) => Ok(Some(kind(&mut tx, &row).await?)),
            None => Ok(None),
        }
    }
    /// Locations require one active same-Workspace Kind and an acyclic live parent.
    /// # Errors
    /// Returns validation, CAS, kind/hierarchy or cross-Workspace errors.
    pub async fn put_location(
        ...
        sqlx::query(if m.revision==Revision::INITIAL {"INSERT INTO locations(location_id,workspace_id,location_kind_id,parent_location_id,display_name,sort_key,description,aliases_json,address_json,latitude,longitude,record_schema,local_revision,created_at_ms,updated_at_ms,state,retired_at_ms,extensions_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,1,?,?,?,?,?,?)"} else {"UPDATE locations SET location_id=?1,workspace_id=?2,location_kind_id=?3,parent_location_id=?4,display_name=?5,sort_key=?6,description=?7,aliases_json=?8,address_json=?9,latitude=?10,longitude=?11,record_schema=1,local_revision=?12,created_at_ms=?13,updated_at_ms=?14,state=?15,retired_at_ms=?16,extensions_json=?17 WHERE location_id=?1 AND workspace_id=?2"})
            .bind(m.id.bytes()).bind(m.workspace_id.bytes()).bind(value.kind_id.bytes()).bind(value.parent_id.map(LocationId::bytes)).bind(&d.display_name).bind(normalize_term(&d.display_name)?).bind(&d.description).bind(canonical(&d.aliases)?).bind(canonical(&value.address)?).bind(value.coordinates.map(|c|c.latitude)).bind(value.coordinates.map(|c|c.longitude)).bind(m.revision.get()).bind(m.created_at.get()).bind(m.updated_at.get()).bind(m.state.sql()).bind(m.retired_at()).bind(canonical(&d.extensions)?).execute(&mut *tx).await?;
        let result = finish(
            &mut tx,
            self.info.device_id,
            Table::Location,
    ...
    /// Bound subjects reserve Workspace/provider/namespace/subject even on tombstones.
    /// # Errors
    ...
    pub async fn put_social_profile(
        ...
        if let Some(handle) = &value.handle {
            ...
            let others=sqlx::query("SELECT social_profile_id,handle FROM social_profiles WHERE workspace_id=? AND provider_id=? AND social_profile_id<>? AND state='active' AND handle IS NOT NULL")
                .bind(m.workspace_id.bytes()).bind(&value.provider_id).bind(m.id.bytes()).fetch_all(&mut *tx).await?;
    ...
    Uuid: From<I>,
{
    ...
    let sql = sqlx::AssertSqlSafe(if m.revision == Revision::INITIAL {
        format!(
            "INSERT INTO {table_name}({id},workspace_id,record_schema,local_revision,created_at_ms,updated_at_ms,state,retired_at_ms,display_name,sort_key,description,aliases_json,extensions_json) VALUES(?,?,1,?,?,?,?,?,?,?,?,?,?)"
        )
    } else {
        format!(
            "UPDATE {table_name} SET {id}=?1,workspace_id=?2,record_schema=1,local_revision=?3,created_at_ms=?4,updated_at_ms=?5,state=?6,retired_at_ms=?7,display_name=?8,sort_key=?9,description=?10,aliases_json=?11,extensions_json=?12 WHERE {id}=?1 AND workspace_id=?2"
        )
    });
    sqlx::query(sql)
        .bind(Uuid::from(m.id).as_bytes().to_vec())
        .bind(m.workspace_id.bytes())
        .bind(m.revision.get())
    ...
    sqlx::query(sqlx::AssertSqlSafe(format!(
        "DELETE FROM {labels_table} WHERE workspace_id=? AND {id}=?"
    )))
    .bind(m.workspace_id.bytes())
    .bind(Uuid::from(m.id).as_bytes().to_vec())
    .execute(&mut *conn)
...
async fn rows(
    ...
    let sql = sqlx::AssertSqlSafe(format!(
        "SELECT * FROM {name} WHERE workspace_id=? AND (? IS NULL OR {column}=?) AND {column}>? AND (? OR state='active') ORDER BY {column} LIMIT ?"
    ));
...
fn workspace(row: &sqlx::sqlite::SqliteRow) -> Result<Workspace> {
    Ok(Workspace {
        meta: metadata(row, "workspace_id")?,
...
async fn party_labels(
    ...
    let sql = sqlx::AssertSqlSafe(format!(
        "SELECT display_label FROM {name} WHERE workspace_id=? AND {column}=? ORDER BY label_key"
    ));
...
async fn person(conn: &mut SqliteConnection, row: &sqlx::sqlite::SqliteRow) -> Result<Person> {
    ...
    details.labels = party_labels(conn, Table::Person, meta.workspace_id, meta.id.uuid()).await?;
    let capabilities=sqlx::query_scalar("SELECT capability_id FROM person_capabilities WHERE workspace_id=? AND person_id=? ORDER BY capability_id").bind(meta.workspace_id.bytes()).bind(meta.id.bytes()).fetch_all(conn).await?.into_iter().collect();
    Ok(Person {
        meta,
        details,
        capabilities,
...
async fn organization(
    ...
    let mut details = details(row)?;
    details.labels =
        party_labels(conn, Table::Organization, meta.workspace_id, meta.id.uuid()).await?;
...
async fn terms(
    ...
    sqlx::query("SELECT term_key,spellings_json FROM location_kind_terms WHERE workspace_id=? AND location_kind_id=? ORDER BY term_key").bind(workspace.bytes()).bind(id.bytes()).fetch_all(conn).await?.iter().map(|r|Ok((r.try_get("term_key")?,json(&r.try_get::<String,_>("spellings_json")?)?))).collect()
}
async fn kind(conn: &mut SqliteConnection, row: &sqlx::sqlite::SqliteRow) -> Result<LocationKind> {
    ...
    let mut aliases: BTreeSet<String> = terms(conn, meta.workspace_id, meta.id)
        .await?
        .into_values()
        .flatten()
        .collect();
...
macro_rules! reads {
    ($get:ident,$list:ident,$id:ident,$record:ident,$table:ident,$decode:ident) => {
        impl LocalLibraryStore {
            ...
            pub async fn $get(&self, workspace: WorkspaceId, id: $id) -> Result<Option<$record>> {
                let mut tx = self.read().await?;
                let rows = rows(
                    &mut tx,
                    Table::$table,
