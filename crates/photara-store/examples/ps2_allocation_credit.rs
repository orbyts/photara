...
//! Credit releases a project's recorded allocation charge, never grants OS space.
...
impl Fixture {
    ...
    fn begin_retire(&mut self, unit: &Unit) -> Result<()> {
        ...
        self.ledger.unpinned(unit.id)?;
            ...
            self.ledger.units.get(&unit.id) == Some(unit) && !unit.retiring,
