...
CREATE INDEX d19_project_ownership_library_state ON photara.project_ownership (library_id,registration_state,project_id);
...
CREATE TABLE photara_identity.project_access_grants (
  ...
  UNIQUE (library_id,project_id,grant_id),
  FOREIGN KEY (library_id,project_id) REFERENCES photara.project_ownership (library_id,project_id) ON DELETE RESTRICT,
...
CREATE UNIQUE INDEX d19_project_access_grants_account_id ON photara_identity.project_access_grants (library_id,project_id,account_id) WHERE account_id IS NOT NULL;

CREATE UNIQUE INDEX d19_project_access_grants_local_principal_id ON photara_identity.project_access_grants (library_id,project_id,local_principal_id) WHERE local_principal_id IS NOT NULL;

CREATE INDEX d19_project_access_grants_account ON photara_identity.project_access_grants (account_id,state,library_id,project_id);
...
CREATE TABLE photara_identity.project_invitations (
  ...
  FOREIGN KEY (library_id,project_id,accepted_grant_id) REFERENCES photara_identity.project_access_grants (library_id,project_id,grant_id) ON DELETE RESTRICT,
...
CREATE UNIQUE INDEX d19_project_invitations_pending ON photara_identity.project_invitations (library_id,project_id,target_account_id) WHERE state='pending';
...
CREATE INDEX d19_project_invitations_fk1 ON photara_identity.project_invitations (library_id,project_id);

CREATE INDEX d19_project_invitations_fk4 ON photara_identity.project_invitations (library_id,project_id,accepted_grant_id);
