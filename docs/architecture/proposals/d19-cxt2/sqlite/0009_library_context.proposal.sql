...
CREATE TABLE library_variables (
  ...
  FOREIGN KEY (workspace_id,variable_id,current_name) REFERENCES library_variable_names (workspace_id,variable_id,name) ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED,
  FOREIGN KEY (workspace_id) REFERENCES workspaces (workspace_id) ON DELETE RESTRICT,
  CHECK (record_schema=1),
  CHECK (updated_at>=created_at)
) STRICT;

CREATE INDEX d19_library_variables_browse ON library_variables (workspace_id,state,current_name,variable_id);
...
CREATE TABLE library_variable_values (
  ...
  UNIQUE (value_id),
  FOREIGN KEY (workspace_id,variable_id) REFERENCES library_variables (workspace_id,variable_id) ON DELETE RESTRICT,
...
CREATE TABLE library_variable_names (
  ...
  FOREIGN KEY (workspace_id,variable_id) REFERENCES library_variables (workspace_id,variable_id) ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE library_expressions (
  ...
  FOREIGN KEY (workspace_id,owner_variable_id) REFERENCES library_variables (workspace_id,variable_id) ON DELETE RESTRICT,
...
CREATE INDEX d19_library_expressions_owner ON library_expressions (workspace_id,owner_variable_id,expression_id);
...
CREATE TABLE library_expression_dependencies (
  ...
  FOREIGN KEY (workspace_id,expression_id) REFERENCES library_expressions (workspace_id,expression_id) ON DELETE RESTRICT,
  FOREIGN KEY (workspace_id,variable_id) REFERENCES library_variables (workspace_id,variable_id) ON DELETE RESTRICT,
  FOREIGN KEY (workspace_id,slot_id) REFERENCES storage_slots (workspace_id,slot_id) ON DELETE RESTRICT,
...
CREATE UNIQUE INDEX d19_library_expression_dependencies_variable_id ON library_expression_dependencies (workspace_id,expression_id,variable_id) WHERE variable_id IS NOT NULL;

CREATE UNIQUE INDEX d19_library_expression_dependencies_slot_id ON library_expression_dependencies (workspace_id,expression_id,slot_id) WHERE slot_id IS NOT NULL;

CREATE INDEX d19_library_variables_fk8 ON library_variables (workspace_id,variable_id,current_name);

CREATE INDEX d19_library_expression_dependencies_fk2 ON library_expression_dependencies (workspace_id,variable_id);

CREATE INDEX d19_library_expression_dependencies_fk3 ON library_expression_dependencies (workspace_id,slot_id);
