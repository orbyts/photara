...
CREATE INDEX d19_project_ownership_library_state ON project_ownership (library_id,registration_state,project_id);
...
CREATE UNIQUE INDEX d19_project_access_grants_account_id ON project_access_grants (library_id,project_id,account_id) WHERE account_id IS NOT NULL;

CREATE UNIQUE INDEX d19_project_access_grants_local_principal_id ON project_access_grants (library_id,project_id,local_principal_id) WHERE local_principal_id IS NOT NULL;

CREATE INDEX d19_project_access_grants_account ON project_access_grants (account_id,state,library_id,project_id);
...
CREATE TABLE project_invitations (
  ...
  FOREIGN KEY (library_id,project_id,accepted_grant_id) REFERENCES project_access_grants (library_id,project_id,grant_id) ON DELETE RESTRICT,
...
CREATE UNIQUE INDEX d19_project_invitations_pending ON project_invitations (library_id,project_id,target_account_id) WHERE state='pending';
...
CREATE INDEX d19_project_invitations_fk1 ON project_invitations (library_id,project_id);

CREATE INDEX d19_project_invitations_fk4 ON project_invitations (library_id,project_id,accepted_grant_id);
