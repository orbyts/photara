...
CREATE INDEX d19_library_variables_browse ON photara.library_variables (workspace_id,state,current_name,variable_id);
...
CREATE TABLE photara.library_variable_values (
  ...
  UNIQUE (value_id),
  FOREIGN KEY (workspace_id,variable_id) REFERENCES photara.library_variables (workspace_id,variable_id) ON DELETE RESTRICT,
...
CREATE TABLE photara.library_variable_names (
  ...
  UNIQUE (workspace_id,variable_id,name),
  FOREIGN KEY (workspace_id,variable_id) REFERENCES photara.library_variables (workspace_id,variable_id) ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED,
  CHECK (name ~ '^[a-z][a-z0-9_]{0,63}$'),
  FOREIGN KEY (workspace_id) REFERENCES photara.workspaces (workspace_id) ON DELETE RESTRICT
);

ALTER TABLE photara.library_variables ADD CONSTRAINT d19_variable_current_name FOREIGN KEY (workspace_id,variable_id,current_name) REFERENCES photara.library_variable_names (workspace_id,variable_id,name) ON DELETE RESTRICT DEFERRABLE INITIALLY DEFERRED;

CREATE INDEX d19_library_variables_current_name ON photara.library_variables (workspace_id,variable_id,current_name);
...
CREATE TABLE photara.library_expressions (
  ...
  FOREIGN KEY (workspace_id,owner_variable_id) REFERENCES photara.library_variables (workspace_id,variable_id) ON DELETE RESTRICT,
...
CREATE INDEX d19_library_expressions_owner ON photara.library_expressions (workspace_id,owner_variable_id,expression_id);
...
CREATE TABLE photara.library_expression_dependencies (
  ...
  PRIMARY KEY (workspace_id,expression_id,ordinal),
  FOREIGN KEY (workspace_id,expression_id) REFERENCES photara.library_expressions (workspace_id,expression_id) ON DELETE RESTRICT,
  FOREIGN KEY (workspace_id,variable_id) REFERENCES photara.library_variables (workspace_id,variable_id) ON DELETE RESTRICT,
  FOREIGN KEY (workspace_id,slot_id) REFERENCES photara.storage_slots (workspace_id,slot_id) ON DELETE RESTRICT,
...
CREATE UNIQUE INDEX d19_library_expression_dependencies_variable_id ON photara.library_expression_dependencies (workspace_id,expression_id,variable_id) WHERE variable_id IS NOT NULL;

CREATE UNIQUE INDEX d19_library_expression_dependencies_slot_id ON photara.library_expression_dependencies (workspace_id,expression_id,slot_id) WHERE slot_id IS NOT NULL;

CREATE INDEX d19_library_expression_dependencies_fk2 ON photara.library_expression_dependencies (workspace_id,variable_id);

CREATE INDEX d19_library_expression_dependencies_fk3 ON photara.library_expression_dependencies (workspace_id,slot_id);
